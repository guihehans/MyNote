...
(function() {
  ...
  window.LoginController = function($scope, $http) {
    $scope.notes = [];
    $scope.toekn = '';
    return $scope.seeNotes = function() {
      var loginRequest;
      ...
      loginRequest.success(function(response, status, headers, config) {
        console.log('success');
        $scope.token = response.token;
        return $http.get('/notes.json?token=' + $scope.token).success(function(data, status, headers, config) {
          console.log(data);
          $scope.notes = data;
          return $scope.info = status;
        });
      });
      return loginRequest.error(function(response, status, headers, config) {
        console.log('error');
        $scope.info = status;
        $scope.notes = [];
        return $scope.token = '';
